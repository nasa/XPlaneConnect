function sendPOSD( posd, ac, socket )
...
%   2. sendPOSD([37.5242422, -122.06899, 2500, 0, 0, 0, 1], 1);
%
...
global clients;
if ~exist('socket', 'var')
    assert(isequal(length(clients) < 2, 1), '[sendPOSD] ERROR: Multiple clients open. You must specify which client to use.');
    if isempty(clients)
    	socket = openUDP();
    else
    	socket = clients(1);
