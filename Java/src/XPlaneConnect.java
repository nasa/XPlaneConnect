...
public class XPlaneConnect implements AutoCloseable {
     ...
     */
    public void setXPlanePort(final int port) {
     ...
     */
    public void setXplaneAddr(final String host) throws UnknownHostException {
        xplaneAddr = InetAddress.getByName(host);
    }

    /**
     * Initializes a new instance of the {@code XPlaneConnect} class using default
     * ports and assuming X-Plane is running on the local machine.
     *
     ...
     * specified timeout using default ports and assuming X-Plane is running on the
     * local machine.
     *
     ...
     *                         receive port.
     */
    public XPlaneConnect(final int timeout) throws SocketException {
     ...
     *                                       resolved.
     */
    public XPlaneConnect(final String xpHost, final int xpPort, final int port)
     ...
     *                         port.
     */
    public XPlaneConnect(final Beacon beacon) throws SocketException {
        this.socket = new DatagramSocket(0);
     ...
     *                                       resolved.
     */
    public XPlaneConnect(final String xpHost, final int xpPort, final int port, final int timeout)
            throws java.net.SocketException, java.net.UnknownHostException {
     ...
     */
    private void sendUDP(final byte[] buffer) throws IOException {
        final DatagramPacket packet = new DatagramPacket(buffer, buffer.length, xplaneAddr, xplanePort);
        socket.send(packet);
    }
     ...
     */
    public void pauseSim(final boolean pause) throws IOException {
        // S I M U LEN VAL
        final byte[] msg = { 0x53, 0x49, 0x4D, 0x55, 0x00, 0x00 };
     ...
     * @throws IOException              If the command cannot be sent.
     */
    public void pauseSim(final int pause) throws IOException {
     ...
     */
    public float[] getDREF(final String dref) throws IOException {
     ...
     * @throws IOException If either the request or the response fails.
     */
    public float[][] getDREFs(final String[] drefs) throws IOException {
        // Preconditions
        ...
        final byte[][] drefBytes = new byte[drefs.length][];
        ...
        final float[][] result = new float[drefs.length][];
    ...
    public void sendDREF(final String dref, final float value) throws IOException {
     ...
     * @throws IOException If the command cannot be sent.
     */
    public void sendDREFs(final String[] drefs, final float[][] values) throws IOException {
        // Preconditions
        if (drefs == null || drefs.length == 0) {
        ...
        for (int i = 0; i < drefs.length; ++i) {
            ...
            // Convert drefs to bytes.
            final byte[] drefBytes = dref.getBytes(StandardCharsets.UTF_8);
            ...
            final ByteBuffer bb = ByteBuffer.allocate(4 * value.length);
            bb.order(ByteOrder.LITTLE_ENDIAN);
     ...
     *                     read.
     */
    public float[] getCTRL(final int ac) throws IOException {
        // Send request
        final ByteArrayOutputStream os = new ByteArrayOutputStream();
     ...
     *               elements will not be changed. To change values in the middle of
     *               the array without affecting the preceding values, set the
     *               preceding values to -998.
     *               </p>
     * @throws IOException If the command cannot be sent.
     */
    public void sendCTRL(final float[] values) throws IOException {
        sendCTRL(values, 0);
    }
     ...
     * @throws IOException If the command cannot be sent.
     */
    public void sendCTRL(final float[] values, final int ac) throws IOException {
        // Preconditions
        if (values == null) {
     ...
     *                     read.
     */
    public double[] getPOSI(final int ac) throws IOException {
        // Send request
        final ByteArrayOutputStream os = new ByteArrayOutputStream();
     ...
     */
    public void sendPOSI(final double[] values) throws IOException {
        sendPOSI(values, 0);
    }
     ...
     * @throws IOException If the command can not be sent.
     */
    public void sendPOSI(final double[] values, final int ac) throws IOException {
        // Preconditions
        if (values == null) {
    ...
    public float[][] readData() throws IOException {
        final byte[] buffer = readUDP();
        final ByteBuffer bb = ByteBuffer.wrap(buffer);
        int cur = 5;
        final int len = bb.get(cur++);
        final float[][] result = new float[bb.get(len)][9];
        for (int i = 0; i < len; ++i) {
            for (int j = 0; j < 9; ++j) {
     ...
     * @throws IOException If the command cannot be sent.
     */
    public void sendDATA(final float[][] data) throws IOException {
        // Preconditions
        ...
        final ByteBuffer bb = ByteBuffer.allocate(4 * 9 * data.length);
        bb.order(ByteOrder.LITTLE_ENDIAN);
        for (int i = 0; i < data.length; ++i) {
            final int rowStart = 9 * 4 * i;
     ...
     * @throws IOException If the command cannot be sent.
     */
    public void selectDATA(final int[] rows) throws IOException {
        // Preconditions
        ...
        // Convert data to bytes
        final ByteBuffer bb = ByteBuffer.allocate(4 * rows.length);
        bb.order(ByteOrder.LITTLE_ENDIAN);
     ...
     * @throws IOException If the command cannot be sent.
     */
    public void sendTEXT(final String msg) throws IOException {
        sendTEXT(msg, -1, -1);
    }
     ...
     * @throws IOException If the command cannot be sent.
     */
    public void sendTEXT(String msg, final int x, final int y) throws IOException {
        // Preconditions
        if (msg == null) {
            msg = "";
        }
     ...
     * @throws IOException If the command cannot be sent.
     */
    public void sendVIEW(final ViewType view) throws IOException {
     ...
     * points are ignored and all points are removed.
     *
     ...
     * @throws IOException If the command cannot be sent.
     */
    public void sendWYPT(final WaypointOp op, final float[] points) throws IOException {
        // Preconditions
        ...
        final ByteBuffer bb = ByteBuffer.allocate(4 * points.length);
        bb.order(ByteOrder.LITTLE_ENDIAN);
        for (final float f : points) {
            bb.putFloat(f);
        }
     ...
     * @throws IOException If the command cannot be sent.
     */
    public void sendCOMM(final String comm) throws IOException {
        // Preconditions
        ...
        final byte[] commBytes = comm.getBytes(StandardCharsets.UTF_8);
     ...
     * @throws IOException If the command cannot be sent.
     */
    public void setCONN(final int port) throws IOException {
        if (port < 0 || port >= 0xFFFF) {
            throw new IllegalArgumentException("Invalid port (must be non-negative and less than 65536).");
        }
